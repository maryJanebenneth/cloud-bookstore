...
app.post('/books', async (req, res) => {
  const { title, author_id, genre, price, stock, published_date } = req.body;
  try {
    const result = await pool.query(
      `INSERT INTO books (title, author_id, genre, price, stock, published_date) 
...
app.post('/orders', async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      // Get book price and check stock
      ...
      if (book.stock < item.quantity) throw new Error(`Insufficient stock for book ${item.book_id}`);
